...
from app.core.speech.base import TranscriptionResult

logger = logging.getLogger("perception.speech")
...
class FunASREngine:
    ...
    def _parse_emotion_tag(raw: str) -> str:
        # SenseVoice: <|EMO_HAPPY|>, <|EMO_SAD|>, <|EMO_ANGRY|>, <|EMO_NEUTRAL|>, <|EMO_UNKNOWN|>
        tags = [
            "EMO_HAPPY",
            "EMO_SAD",
            "EMO_ANGRY",
        ...
        for t in tags:
            if f"<|{t}|>" in raw:
                return t.removeprefix("EMO_").lower()
        return "neutral"
