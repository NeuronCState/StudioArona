...
def get_all_tools(include_disabled: bool = False) -> list[BaseTool]:
    """返回所有已注册的 Tool 实例。"""
    client = _get_client()
    ...
    tools = [
        ...
        # Studio Arona Center (authenticated as the current user)
        StudioScheduleTool(client=client),
        StudioFeedTool(client=client),
        StudioMonitorTool(client=client),
        StudioStatusTool(client=client),
        StudioMemoryTool(client=client),
    ]
    from config.studio import disabled_tools

    disabled = set() if include_disabled else disabled_tools()
