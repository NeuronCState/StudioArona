...
const navItems = [
  ...
  { to: "/config", icon: Settings2, i18nKey: "sidebar.config" },
];

/** 工作室服务导航项 */
const studioNavItems = [
  { to: "/vms", icon: Monitor, i18nKey: "sidebar.vms", needConnection: true },
  { to: "/nas", icon: HardDrive, i18nKey: "sidebar.nas", needConnection: true },
  {
    to: "/admin",
    icon: ShieldCheck,
...
const routePrefetchMap: Record<
  string,
  Array<{ key: readonly unknown[]; fetcher: () => Promise<unknown> }>
> = {
  "/feeds": [
  ...
  "/schedule": [
    {
      ...
      fetcher: () => api.get("/schedules?upcoming=true"),
    },
  ],
  "/admin": [
  ...
  "/vms": [{ key: ["vms"], fetcher: () => api.get("/vms") }],
};
...
export function StudioSidebar() {
  ...
  const visibleStudioItems = studioNavItems.filter(
  ...
  return (
    ...
    >
        ...
        >
          ...
          <AnimatePresence>
            {linuxGroupOpen && (
              ...
              >
                {visibleStudioItems.map(
                  ({ to, icon: Icon, i18nKey, needConnection }) => {
      ...
      <NavLink
        to="/settings"
        aria-label="个人设置"
        className="group flex items-center gap-2 border-t px-4 py-3 transition-colors hover:bg-[var(--studio-accent-soft)]"
        style={{ borderColor: "var(--studio-border)" }}
      >
        <div className="relative">
          <Avatar
            src={
              (user?.preferences as Record<string, string> | undefined)
                ?.avatar_url
            }
        ...
        <UserCog
          size={16}
          className="text-[var(--studio-text-muted)] transition-colors group-hover:text-[var(--studio-accent)]"
        />
      </NavLink>
    </aside>
  );
