...
import { Tooltip, Popover, Dropdown, Dialog, Drawer, Button } from '@javis/ui-kit';
  ...
  render: () => (
    <Popover trigger={<Button variant="secondary" size="sm">Open Popover</Button>}>
      <div className="p-3">
        ...
        <p className="mt-1 text-xs text-[var(--color-text-secondary)]">This is popover content.</p>
      </div>
    </Popover>
  ),
};
...
export const DropdownExample: StoryObj = {
  render: () => (
    <Dropdown
      items={[
        { label: 'Settings', onClick: () => alert('Settings'), icon: <Settings size={14} /> },
      ...
      <>
        ...
        <Drawer open={open} onClose={() => setOpen(false)} title="Settings">
          <div className="space-y-4">
            <p className="text-sm text-[var(--color-text-secondary)]">Drawer content goes here.</p>
