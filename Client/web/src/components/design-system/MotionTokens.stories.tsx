...
function MotionTokensDemo() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', maxWidth: 720 }}>
      {/* ---- Easing curves ---- */}
      <section>
        <h2 style={{ fontSize: '1.2rem', marginBottom: '0.75rem', color: 'var(--color-text-primary)' }}>
          Easing Curves
        </h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {(Object.keys(EASING) as Array<keyof typeof EASING & string>).map((name) => (
            <div
              key={name}
              ...
              <div style={{ position: 'relative', height: 32, flex: 1 }}>
                <div
                  ...
                  style={{
                    ...
                    transition: activeEasing === name
                      ? `transform ${DURATION.base}ms ${bezierCSS(EASING[name])}`
                      : 'none',
                    transform: activeEasing === name ? 'translateX(calc(100% - 24px))' : 'none',
                    cursor: 'pointer',
                  }}
                />
              </div>
      ...
      <section>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {(Object.keys(DURATION) as Array<keyof typeof DURATION & string>).map((name) => (
              ...
              <div style={{ position: 'relative', height: 24, flex: 1 }}>
                <div
                  onClick={() => {
                    ...
                    setTimeout(() => setActiveDuration(null), DURATION[name] + 200);
                  }}
                  style={{
                    height: '100%',
                    borderRadius: 'var(--radius-sm)',
                    backgroundColor: 'var(--color-accent-soft)',
                    transition: activeDuration === name
