...
describe("AgentMessageItem — error rendering", () => {
  ...
  it("shows the normal assistant bubble for healthy messages", () => {
    ...
    expect(
      screen.getByText("partial content before the failure"),
    ).toBeInTheDocument();
...
describe("AgentErrorToast", () => {
  ...
  it("renders nothing when message is null", () => {
    ...
    expect(
      container.querySelector('[data-testid="agent-error-toast"]'),
    ).toBeNull();
  });
  ...
  it("auto-dismisses after the configured duration", () => {
    ...
    const onDismiss = vi.fn();
    render(
      <AgentErrorToast message="boom" onDismiss={onDismiss} durationMs={500} />,
    );
