...
export function CreateVmForm({ onClose, onSuccess }: CreateVmFormProps) {
  const {
    ...
    defaultValues: { spec_cpu: 2, spec_ram_mb: 4096, spec_disk_gb: 50, hypervisor: 'mock' },
  });
  ...
  return (
    <div className="card motion-slide-up space-y-4">
      ...
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="space-y-3">
        ...
        <div>
          <label className="mb-1 block text-xs text-text-secondary">CPU 核数</label>
          <input
            type="range"
            min={1}
            max={8}
        ...
        <div>
          <label className="mb-1 block text-xs text-text-secondary">内存 (MB)</label>
          <input
            type="range"
            min={2048}
            max={16384}
        ...
        <div>
          <label className="mb-1 block text-xs text-text-secondary">硬盘 (GB)</label>
          <input {...register('spec_disk_gb')} className="input" type="number" min={10} max={200} />
        </div>

        <div>
          <label className="mb-1 block text-xs text-text-secondary">Hypervisor</label>
