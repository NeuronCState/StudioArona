    ...
    <>
      ...
      >
        ...
        {vms && vms.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {vms.map((vm) => (
              <div key={vm.id} className="card cursor-pointer" onClick={() => setSelectedVm(vm)}>
                ...
                <div className="flex gap-4 text-xs text-[var(--color-text-secondary)]">
                  ...
                  <span>{vm.spec_ram_mb != null ? `${(vm.spec_ram_mb / 1024).toFixed(1)} GB` : '-'}</span>
                  <span>{vm.spec_disk_gb != null ? `${vm.spec_disk_gb} GB` : '-'}</span>
                  <span className="text-[var(--color-text-muted)]">{vm.hypervisor}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <CardEmpty
            ...
            icon={<Terminal size={32} className="text-[var(--color-text-muted)]" />}
          />
        )}
      </PageLayout>
