...
function matchCategory(
  ...
  (DOMAIN_KEYWORD_BOOST[category] ?? []).forEach((k) => keywords.add(k.toLowerCase()));
...
const DOMAIN_SEARCH_FALLBACK: Record<string, string[]> = {
  ...
  'data-ai': ['machine-learning', 'data-analysis', 'llm', 'rag', 'data-engineering'],
  design: ['frontend-design', 'figma', 'tailwind', 'ui-design', 'design-system'],
  databases: ['postgres', 'sql', 'mongodb', 'nosql', 'database'],
  'testing-security': ['testing', 'security-audit', 'jest', 'pytest', 'penetration'],
  documentation: ['documentation', 'readme', 'technical-docs', 'guide'],
  'content-media': ['content-creation', 'blog', 'video', 'seo', 'media'],
  business: ['sales', 'marketing', 'finance', 'project-management', 'ecommerce'],
...
const DOMAIN_KEYWORD_BOOST: Record<string, string[]> = {
  databases: ['postgres', 'postgresql', 'sql', 'mysql', 'mongodb', 'nosql', 'redis', 'sqlite', 'database', 'sql-database', 'nosql-database'],
  devops: ['docker', 'kubernetes', 'k8s', 'terraform', 'ansible', 'helm', 'ci', 'cd', 'cicd', 'cloud', 'aws', 'gcp', 'azure', 'linux', 'bash', 'shell', 'nginx', 'monitor', 'container'],
  'data-ai': ['machine-learning', 'ml', 'ai', 'llm', 'gpt', 'rag', 'agent', 'data', 'analytics', 'pandas', 'numpy', 'embedding', 'vector', 'data-analysis', 'data-engineering', 'learning', 'neural', 'model'],
  development: ['frontend', 'backend', 'mobile', 'web', 'full-stack', 'developer', 'programming', 'code', 'coding', 'lib', 'sdk', 'api', 'framework', 'package', 'scripting'],
  'testing-security': ['test', 'testing', 'security', 'audit', 'jest', 'pytest', 'pentest', 'vuln', 'ssl', 'encrypt', 'auth', 'oauth', 'jwt'],
  documentation: ['doc', 'docs', 'readme', 'guide', 'tutorial', 'wiki', 'markdown', 'documentation', 'knowledge', 'education'],
  'content-media': ['blog', 'article', 'video', 'audio', 'image', 'media', 'content', 'creation', 'seo', 'social', 'youtube'],
  business: ['business', 'sales', 'marketing', 'finance', 'hr', 'ecommerce', 'payment', 'project-management', 'legal', 'contract'],
  design: ['design', 'ui', 'ux', 'figma', 'css', 'tailwind', 'style', 'theme', 'color', 'font', 'icon', 'svg', 'frontend-design'],
  tools: ['tool', 'cli', 'git', 'automation', 'ide', 'productivity', 'utility', 'helper', 'script', 'workflow', 'bot'],
};
...
export function MarketplaceTab() {
  ...
  const [trendingWindow, setTrendingWindow] = useState<'all' | 'week' | 'month'>('all');
  ...
  const { data: categoriesData } = useQuery({
    ...
    queryFn: () => api.get<CategoriesResponse>('/skills/marketplace/categories'),
  ...
  const { data: installedData } = useQuery({
    ...
    queryFn: () => api.get<{ skills: MarketSkill[]; slugs: string[]; total: number }>(
  ...
  const {
    ...
    queryFn: () => api.get<{ skills: MarketSkill[]; total: number; cached: boolean }>(
  ...
  const currentData: SearchResponse | undefined = useMemo(() => {
    ...
    if (viewMode === 'trending' && trendingData) {
      ...
      if (viewMode === 'category' && category && categoriesData?.categories) {
        baseSkills = matchCategory(rawData.skills, category, categoriesData.categories);
      } else {
  ...
  return (
    <div className="space-y-4">
          ...
          {([
            ...
            return (
              <button
                key={id}
                onClick={() => {
                  if (id === 'trending') { setViewMode('trending'); setCategory(null); setSearch(''); }
                  else if (id === 'category') { setViewMode('category'); setSearch(''); }
                  else { setViewMode('search'); setCategory(null); }
                }}
                className={cn(
      ...
      {viewMode === 'category' && (
        <CategoryTabs
          selected={category}
          options={availableDomains.map((d) => ({ id: d, label: domainLabel(d, categoriesData?.categories) }))}
      ...
      <AnimatePresence mode="wait" initial={false}>
        {isPending ? (
          ...
          >
            ...
            <StaggerList
              className="grid grid-cols-1 gap-4 md:grid-cols-2"
              staggerKey={((viewMode + (trendingWindow ?? '') + (category ?? '') + (search ?? '') + '-' + page) as 'list' | 'page')}
            >
