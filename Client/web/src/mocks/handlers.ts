...
export const handlers = [
  ...
  http.patch('/api/me', async ({ request }) => {
    ...
    if (body.display_name !== undefined) mockUsers[0].display_name = body.display_name;
    return HttpResponse.json({
      id: mockUsers[0].id,
  ...
  http.get('/api/schedules', ({ request }) => {
    ...
    const filtered = scope ? mockSchedules.filter((s) => s.source === scope) : mockSchedules;
  ...
  http.post('/api/page-monitors', async ({ request }) => {
    const body = (await request.json()) as { url: string; label: string; css_selector?: string };
    const monitor = {
      id: `pm_${Date.now()}`,
      user_id: 'u_zhang',
      url: body.url,
  ...
  http.patch('/api/page-monitors/:monitorId', async ({ request, params }) => {
    const body = (await request.json()) as { label?: string; css_selector?: string; enabled?: boolean };
    const idx = mockPageMonitors.findIndex((m) => m.id === params.monitorId);
    if (idx < 0) return new HttpResponse(null, { status: 404 });
    mockPageMonitors[idx] = { ...mockPageMonitors[idx], ...body, updated_at: new Date().toISOString() };
